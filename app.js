/* ═══════════════════════════════════════════════════════════
   LADY ART NAILS — app.js · award edition
   ═══════════════════════════════════════════════════════════
   ★★★ ВАЖНО: вставь реальный WhatsApp-номер салона ★★★
   Формат: код страны + номер БЕЗ плюса, пробелов и тире.
   Например для +7 702 123-45-67 → "77021234567"
   Реальный номер также продублируй в index.html (секция «Контакты»).
*/
const WHATSAPP_NUMBER = "77021111111"; // ← ЗАМЕНИТЬ на реальный номер

const SERVICES = [
  { id: "manicure-gel", tag: "хит", name: "Комби-маникюр + гель-лак", desc: "Снятие, аккуратная обработка, идеальный блик и покрытие под кутикулу. Без боли.", price: 8000, dur: "1,5–2 ч", img: "https://images.unsplash.com/photo-1604654894610-df63bc536371?q=80&w=700&auto=format&fit=crop" },
  { id: "extension", tag: "топ", name: "Наращивание ногтей", desc: "Длина и форма мечты: миндаль, квадрат, балерина. Прочная архитектура от Марины.", price: 12000, dur: "2,5–3 ч", img: "https://images.unsplash.com/photo-1632345031435-8727f6897d53?q=80&w=700&auto=format&fit=crop" },
  { id: "correction", tag: "", name: "Коррекция наращивания", desc: "Обновление длины, коррекция «клюющих», ремонт. Ногти снова как новые.", price: 9000, dur: "~2 ч", img: "https://images.unsplash.com/photo-1610992015732-2449b76344bc?q=80&w=700&auto=format&fit=crop" },
  { id: "pedicure", tag: "уход", name: "Педикюр + покрытие", desc: "Обработка стоп, кутикула, гель-лак. Лёгкость и ухоженность надолго.", price: 10000, dur: "~2 ч", img: "https://images.unsplash.com/photo-1519019121902-f27deb4146ae?q=80&w=700&auto=format&fit=crop" },
  { id: "reconstruction", tag: "фишка", name: "Реконструкция ногтей на ногах", desc: "Фирменная услуга Марины перед отпуском: идеальные ногти даже в сложной ситуации.", price: 10000, dur: "~1,5 ч", img: "https://images.unsplash.com/photo-1607779097040-26e80aa78e66?q=80&w=700&auto=format&fit=crop" },
  { id: "design", tag: "арт", name: "Дизайн / декоративное наращивание", desc: "Френч, втирка, стразы, роспись, корейский арт. Подберём под твой муд.", price: 2000, dur: "+30 мин", img: "https://images.unsplash.com/photo-1522337660859-02fbefca4702?q=80&w=700&auto=format&fit=crop", from: true },
];

const MASTERS = ["Любой мастер", "Марина", "Наталья", "Александра"];
...
(() => {
  ...
  plist.addEventListener("pointerover", e => {
    ...
    if (row && img.getAttribute("src") !== row.dataset.img) { img.src = row.dataset.img; img.alt = ""; }
  });
...
/* ---------- booking: state ---------- */
const fService = $("#fService"), fMaster = $("#fMaster"), fDate = $("#fDate"),
      fName = $("#fName"), fPhone = $("#fPhone"), priceOut = $("#priceOut"),
      ticketMain = $("#ticketMain"), ticketSub = $("#ticketSub"),
      slotsEl = $("#slots"), bookForm = $("#bookForm");
let selTime = "";

if (fService && fDate && slotsEl) {
  ...
  fDate.min = localISO(new Date());
  fDate.max = localISO(maxD);

  const currentService = () => SERVICES.find(s => s.id === fService.value) || SERVICES[0];
  const isPast = t => {
    if (!fDate.value || fDate.value !== localISO(new Date())) return false;
  ...
  const refreshSummary = () => {
    ...
    const price = (s.from ? "от " : "") + fmt(s.price);
    if (priceOut) priceOut.textContent = price;
    const d = fDate.value
      ? new Date(fDate.value + "T00:00").toLocaleDateString("ru-RU", { day: "numeric", month: "long", weekday: "short" })
      : "дата?";
    if (ticketMain) ticketMain.textContent = `${s.name} · ${selTime || "время?"}`;
    if (ticketSub) ticketSub.textContent = `${d} · ${fMaster ? fMaster.value : "любой мастер"} · ${price}`;
  };
  ...
  [fService, fMaster].forEach(el => el && el.addEventListener("change", refreshSummary));
  fDate.addEventListener("change", () => {
    if (selTime && isPast(selTime)) { selTime = ""; renderSlots(); }
    refreshSummary();
  });

  /* ---------- open booking with preselected service / master ---------- */
  const openBooking = (serviceId, master) => {
    if (serviceId) fService.value = serviceId;
    if (master && fMaster) fMaster.value = master;
    refreshSummary();
    const menu = $("#mmenu");
    ...
    setTimeout(() => fName && fName.focus({ preventScroll: true }), RM ? 100 : 800);
  };
  ...
  if (bookForm) bookForm.addEventListener("submit", e => {
    ...
    if (!fDate.value) { showToast("Выбери дату визита 📅"); fDate.focus(); return; }
    if (!selTime) { showToast("Выбери время — работаем 09:00–20:00 ⏰"); return; }

    const dateRu = new Date(fDate.value + "T00:00").toLocaleDateString("ru-RU", { day: "numeric", month: "long", weekday: "long" });
    const lines = [
      "Здравствуйте, Lady Art Nails! 💅 Хочу записаться:",
      `• Имя: ${name}`,
    ...
    if (ticketMain) {
      const old = ticketMain.textContent;
      ticketMain.textContent = "Отправлено ✓ проверь WhatsApp";
      setTimeout(() => { ticketMain.textContent = old; refreshSummary(); }, 4000);
    }
  });
}
...
(() => {
  ...
  document.addEventListener("pointerover", e => {
    ...
    const act = e.target.closest("a,button,.slot,.shot,summary,input,select");
